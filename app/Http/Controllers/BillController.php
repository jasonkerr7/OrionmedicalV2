...
class BillController extends Controller
{
    ...
    public function getPatientBill($id)
   {
    ...
    $receivables = 0;
      ...
      $bills_patient              = Bill::where('patient_id', $patientid)->orderBy('date', 'desc')->get();
      $paiditems_patient          = Payments::where('PatientID', $patientid)->get();
      
          $payables_patient = 0;
          $receivables_patient = 0;
          $outstanding_patient=0;
  
          foreach($bills_patient as $bill_patient)
         {
              $payables_patient += ($bill_patient->rate * $bill_patient->quantity);
         }
  
         foreach($paiditems_patient as $paiditem_patient)
         {
              $receivables_patient += ($paiditem_patient->AmountReceived);
         }
  
         //dd($receivables);
  
         $outstanding_patient = ($payables_patient - $receivables_patient);


    $receiptmodes   = DB::table('receipt_mode')->get();
    $services       = ServiceCharge::where('status','Active')->get();
    return view('billing.invoice', compact('patients','outstanding_patient','visitdetails','bills','receiptmodes','balances','services','outstanding','payables','receivables','generalservices'));
}
 ...
 public function printBill($id)
   {
    ...
    $bills     =  Bill::where('visit_id' ,'=', $id)->where('rate','>',0)->orderBy('date', 'desc')->get();
    $paiditems          = Payments::where('EventID', $id)->get();
...
public function index()
  ...
  public function insuranceportal()
   ...
   else
   {
    ...
    $receiptmodes   = DB::table('receipt_mode')->get();
    $services       = ServiceCharge::where('status','Active')->get();
    return view('claims.vet', compact('url','patients','drugs','diagnosis','visitdetails','bills','mydiagnosis','receiptmodes','balances','services','outstanding','payables','receivables','generalservices'));
}
    ...
    public function payments()
   {
    ...
    $billcount = 0;//BillMaster::where('note', 'Unpaid')->count();
    $receiptcount = 0; //BillMaster::where('note', 'Paid')->count();
    $receiptmodes=DB::table('receipt_mode')->get();
    ...
    public function reversePayment()
    {
      ...
      if(Auth::user()->getRole()== "System Admin")
      {
        if(Hash::check(Input::get("mypassword"), Auth::user()->password()))
         ...
         {
             $ID = Input::get("ID");
            //$affectedRows = Payments::where('id', '=', $ID)->delete();

            $items = Payments::where('id', '=', $ID)->first();
            $duplicateReceipt = $items->replicate();
            $duplicateReceipt->AmountReceived = ($items->AmountReceived * -1);
            $duplicateReceipt->CreateDate = Carbon::now();
            $duplicateReceipt->CreatedBy  = Auth::user()->getNameOrUsername(); 
           // $duplicateReceipt->EventID    = uniqid();
            //$duplicateReceipt->save();
 
             if( $duplicateReceipt->save())
             {
                 $ini   = array('OK'=>'OK');
    ...
    public function dashboard()
   {
    ...
    public function discountInvoiceItem()
    {
            $itemid  = Input::get("invoice_id");
            $discount_value  = Input::get("discount_charge");

            $affectedRows2 = Bill::where('id',  $itemid)->update(array('discount'  => $discount_value));
    ...
    try
    {
            ...
            $bills=Bill::where('visit_id' , $opd_number)->orderBy('date', 'ASC')->get();
    ...
    public function fetchbillTotal()
    {
      //dd($opd_id);
    ...
    foreach($bills as $bill)
      {
            $payables += (($bill->rate * $bill->quantity) - $bill->discount);
      }
    ...
    foreach($paiditems as $paiditem)
      ...
      $out = number_format($outstanding, 2, '.', '');
      $pab = number_format($payables, 2, '.', '');
      $rcv = number_format($receivables, 2, '.', '');

      $added_response = array('outstanding' => $out,'payables'=>$pab,'receivables'=>$rcv);
      return Response::json($added_response);
    }
...
//ZOLMITRIPTAN 2.5MG  2.5 MG STAT AT THE ONSET OF THE HEADACHE, REPEAT AFTER 2HRS IF THE HEADACHE DOES NOT SUBSIDE. NOT TO
...

      {
       ...
       //{
        ...
        //$setOustandingZero = Payments::where('EventID', '=',$request->input('visit_id'))->update(array('Outstanding' => 0));
        ...
        $payments->Outstanding =  $request->input('outstanding') - $request->input('amountreceived');
