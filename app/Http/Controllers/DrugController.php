...
class DrugController extends Controller
{
    ...
    public function dashboard()
   {
    ...
    public function getPendingMedication($id)
    {
        ...
        $druglists   = Drug::where('status','Active')->orderBy('name', 'ASC')->get();
        $periods=DrugPeriod::orderBy('type', 'ASC')->get();
        $application=DrugApplication::get();
        $frequency=DrugFrequency::orderBy('type', 'ASC')->get();
        $dosage=DrugDosage::get();
       ...
       foreach($paiditems as $paiditem)
       {
        ...
        //dd($totalcost);
        return view('pharmacy.dispense',compact('company','druglists','periods','application','application','frequency','dosage','payables','receivables','outstanding','mydiagnosis','mycomplaints','totaldispensed','exclusions','patientid','visit_details','patients','prescriptions','dispensed','drugs','totalcost'));
   ...
   //dd($drug_checked);
        ...
        if(is_array($drug_checked))
        {
          foreach($drug_checked as $drug_checked)
          {
                ...
                $checkavailability = Drug::where('id', $affectedRows->drug_id)->first();
                ...
                foreach ($getuploaddetails as $value) 
                    {
                        ...
                        $affectedRows2   = Drug::where('id', $value->name)->update(array('unit_price' => $value->unit_price,'supplier' => $value->supplier,'expiry_date' => $value->expiry_date));   


                        $affectedRows3   = DrugStock::where('id', $value->id)->update(array('upload_state' => 'Uploaded','uploaded_on' => Carbon::now(),'uploaded_by' => Auth::user()->getNameOrUsername()));
                        //$affectedRows3   = DrugStock::where('id', $value->id)->update(array());
                        //$affectedRows3   = DrugStock::where('id', $value->id)->update(array());
