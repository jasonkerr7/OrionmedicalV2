...
class HomeController extends Controller
{
    ...
    public function index()
    {
       ...
       $views = OPD::where('payercode','Private')->whereRaw("created_on >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("count(*) as no_of_visits,0"),DB::raw("DATE_FORMAT(created_on,'%d-%M') as day_of_month")]);
       $views2 = OPD::where('payercode','Corporate')->whereRaw("created_on >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("count(*) as no_of_visits,0"),DB::raw("DATE_FORMAT(created_on,'%d-%M') as day_of_month")]);
       $views3 = OPD::where('payercode','Health Insurance')->whereRaw("created_on >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("count(*) as no_of_visits,0"),DB::raw("DATE_FORMAT(created_on,'%d-%M') as day_of_month")]);
       ...
       $views = Bill::where('payercode','Private')->whereRaw("date >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("sum(quantity*rate) as billamount"),DB::raw("DATE_FORMAT(date,'%d-%M') as day_of_month")]);
       $views2 = Bill::where('payercode','Corporate')->whereRaw("date >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("sum(quantity*rate) as billamount"),DB::raw("DATE_FORMAT(date,'%d-%M') as day_of_month")]);
       $views3 = Bill::where('payercode','Health Insurance')->whereRaw("date >= last_day(now()) + interval 1 day - interval 1 month")->groupBy(DB::raw('day_of_month'))->orderby('day_of_month','asc')->limit(30)->get([DB::raw("sum(quantity*rate) as billamount"),DB::raw("DATE_FORMAT(date,'%d-%M') as day_of_month")]);
        ...
        // Services sources chart
             ...
             ->select(DB::raw("DATE_FORMAT(date,'%Y-%m') as day_of_month"), DB::raw("sum(quantity*rate) as billamount"))
            ->where('category','Dental')
             ->groupBy('day_of_month')
             ->orderby('day_of_month','asc')
             ->limit(60)
        ...
        foreach($myimagingbills as $imaging)
       {
        ...
        $birthdays =  Customer::whereMonth('date_of_birth', '=', $current_date->month)->whereDay('date_of_birth', '=', $current_date->day)->get(); //Customer::where('accounttype','Private')->whereRaw('DAYOFYEAR(curdate()) == DAYOFYEAR(date_of_birth) AND DAYOFYEAR(curdate()) ==  dayofyear(date_of_birth)')->get();
       

        return View('pages.dashboard', compact('assignees','birthdays','payments','businesssourcecharts','drugs','categorychartsutilization','categorycharts','billscharts','myvisits','company','bills','opdbills','chartjs','dayscharts','diagnosischartjs','medicalchartjs','imagingbills','customercount','events','patients','visits','visitrate','pharmacybills','labbills'));
    }
