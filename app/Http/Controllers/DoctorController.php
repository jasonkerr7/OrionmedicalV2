...
use OrionMedical\Models\LabPanel;
...
class DoctorController extends Controller
{
   ...
   public function loadAllDiagnosis()
    ...
    {   
        ...
        $images           = Images::where('accountnumber' , $visit_details->patient_id)->get();
        $investigations   = ServiceCharge::where('department','Laboratory')->orwhere('department','Radiology')->orderBy('type', 'ASC')->get();
        $complaints       = Complaint::where('speciality','General')->orderBy('type', 'ASC')->get() ?: new Complaint;
        $application      = DrugApplication::get();
        $frequency        = DrugFrequency::orderBy('type', 'ASC')->get();
        $dosage           = DrugDosage::orderBy('type', 'ASC')->get();
        $drugs            = Drug::where('status','Active')->orderBy('name', 'ASC')->get();
        ...
        //Timeline
        $mycomplaints = PatientComplaint::where('visitid' ,'=', $id)->first() ?: new PatientComplaint;
        $myhistories = PatientHistory::where('patientid' ,'=', $visit_details->patient_id)->first() ?: new PatientHistory;
        ...
        $mylabstotal = PatientInvestigation::where('visitid' ,'=', $id)->count();
        $mylabsready = PatientInvestigation::where('visitid' ,'=', $id)->where('status','Ready')->count();

        
        $myoldmeds = Prescription::where('patientid' , $visit_details->patient_id)->groupby('visitid')->get();

       // dd($expcomplaints);

        $views = DB::table('patient_vitals')
             ->select(DB::raw("DATE_FORMAT(created_on,'%H:%i, %d-%M-%Y') as period"),DB::raw("visit_id,temperature,pulse_rate,weight,sbp,dbp"))
        ...
        $oldvisits  =   OPD::where('patient_id' ,'=', $visit_details->patient_id)->orderby('created_on','desc')->get();
        $servicetypes     = ServiceCharge::orderBy('type', 'ASC')->where('department','OPD')->get();
        ...
        $prescriptions            = Prescription::where('visitid', $id)->get();
        ...
        $myortho = PatientOrtho::where('visitid' ,'=', $id)->first() ?: new PatientOrtho;

        //dd($myortho);

        $labresults          = LabTestResult::where('labid', $id)->where('result','<>',null)->get()->groupBy(function($item) 
        {
          return $item->type;
        });

        $myoldlabs = PatientInvestigation::where('patientid' , $visit_details->patient_id)->orderby('created_on','desc')->get()->groupBy(function($item) 
        {
          return $item->visitid;
        });

        $myolddrugs = Prescription::where('patientid' , $visit_details->patient_id)->orderby('created_on','desc')->get()->groupBy(function($item) 
        {
          return $item->visitid;
        });
      ...
      //dd($labresults);
       ...
       return view('doctor.review', compact('mype','lenstypes','myortho','lenstreatments','eyefindings','ocularfindings','prescriptions','defaultheight','labresults','myoldlabs','myolddrugs','mynursenotes','wards','mylabstotal','mylabsready','ipdservices','branches','accounttype','referals','payables','receivables','outstanding','mydoctorplan','continuation','vitalcharts','expcomplaints','expdirectquestion','myplan','pe_constitutional','oldvisits','pe_heart','pe_neck','pe_breast','pe_psychological','pe_neuropsychiatric','pe_musculoskeletal','pe_extremities','pe_gynecology','pe_abdominal','pe_genitourinary','pe_gastro','pe_HEENT','pe_respiratory','patients','bills','payables','visittypes','servicetypes','pastmedicalhx','familyhx','allergichx','reproductivehx','surgicalhx','medicationhx','vacinnationhx','socialhx','mydiagnosis','myvitals','myhistories','mylabs','mydrugs','myros','mycomplaints','triage','complaintperiods','visit_details','ros_throat','ros_eyes','ros_ears','ros_nose','ros_skin','ros_head','ros_constitutional','ros_respiratory','ros_cardiovasular','ros_gastro','ros_gynecology','ros_genitourinary','ros_endocrine','ros_musculoskeletal','ros_peripheral_vascular','ros_hematology','ros_neuropsychiatric'))
        ->with('doctors',$doctors)
        ->with('diagnosis',$diagnosis)
        ->with('treatments',$treatments)
        ->with('drugs',$drugs)
    ...
    {   
      ...
      return view('doctor.walkin', compact('patients','drugs','diagnosis','visitdetails','bills','mydiagnosis','receiptmodes','balances','services','outstanding','payables','receivables','generalservices'));
    }
    ...
    public function dentalMonthlyVisit()
    {   
    ...
    {   
       ...
       // dd($expcomplaints);
        ...
        $branches = Branch::get();
         $mydoctorplan = PatientAssessment::where('visit_id' ,'=', $id)->first() ?: new PatientAssessment;
        $continuation = ContinuationSheet::where('visit_id' ,'=', $id)->orderby('created_on','desc')->first() ?: new ContinuationSheet;
        $continuationop = ContinuationSheetOP::where('visit_id' ,'=', $id)->orderby('created_on','desc')->first() ?: new ContinuationSheetOP;
        $wards = Ward::get();
         $ipdservices     = ServiceCharge::orderBy('type', 'ASC')->where('department','IPD')->get();
      ...
      $myoldlabs = PatientInvestigation::where('patientid' , $visit_details->patient_id)->where('type','Laboratory')->where('created_on','>','2019-11-01')->groupby('visitid')->get();
        $myoldmeds = Prescription::where('patientid' , $visit_details->patient_id)->groupby('visitid')->get();
    ...
    {   
        ...
        $today     = Carbon::now()->format('Y-m-d').'%';
        $patients  = OPD::where('consultation_type','like','%EYE%')->orwhere('consultation_type','like','%EXAM%')->orwhere('consultation_type','like','%PRE-EMPLOYMENT%')->where('created_on', 'like', $today)->orderBy('created_on', 'DESC')->paginate(30);
    ...
    {   
        ...
        $mycomplaints = PatientComplaint::where('visitid' ,'=', $id)->get();
        $myhistories = PatientHistory::where('patientid' ,'=', $visit_details->patient_id)->latest('created_on')->first() ?: new PatientHistory;
...
public function laboratory()
    ...
    public function getDischarged()
   {
    ...
    {     
           ...
           $prescription->drug_application = strtoUpper(Input::get("drug_application"));
           $prescription->drug_frequency   = $drug->barcode;
           ...
           // OneSignal::sendNotificationToUser($drug->name."(Medication) has been requested from ".Auth::user()->getNameOrUsername(), '1a76f6fb-9f6f-4e7b-a1b0-ff99999b41a4', $url = null, $data = null, $buttons = null, $schedule = null);
           // OneSignal::sendNotificationToUser($drug->name."(Medication) has been requested from ".Auth::user()->getNameOrUsername(), '1a76f6fb-9f6f-4e7b-a1b0-ff99999b41a4', $url = null, $data = null, $buttons = null, $schedule = null);
    ...
    public function addMedicationExtra()
    {     
            $drug =  Drug::where('id',Input::get("medication"))->first();
            $diagnosis   =  PatientDiagnosis::where('visitid',Input::get("opd_number"))->first();
            $transactionid = uniqid(20);
           ...
           $prescription->drug_cost   = $drug->unit_price * $margin;
           $prescription->drug_ini_cost   = $drug->unit_price;
           $prescription->drug_quantity    = Input::get("drug_quantity");
           $prescription->drug_dosage      = Input::get("drug_dosage");
           ...
           $bill->fullname          = Input::get("fullname");
           $bill->item_name         = $drug->name;
           $bill->quantity          = Input::get("drug_quantity");
           $bill->rate              = $drug->unit_price * $margin;
           $bill->amount            = $drug->unit_price * $margin;
           $bill->note              = 'Unpaid';
           $bill->category          = 'Pharmacy';
...
public function generateStaffID()
{
    ...
    {     
                     ...
                     $myinvestigations = Input::get("investigation");
                     $investigation_amount = 0;

                      if(Str::contains($myinvestigations, 'PANEL'))
                      {
                        $labs = LabPanel::where('panel',$myinvestigations)->get();

                        foreach($labs as $panellab)
                        {
                          $investigation_amount = $panellab->price;
                                                              ...
                                                              $investigation->patientid       = Input::get("patient_id");
                                                              $investigation->visitid         = Input::get("opd_number");
                                                              $investigation->investigation   = $panellab->lab;
                                                              $investigation->remark          = Input::get("remark");
                                                              ...
                                                              // {
                                                              //   OneSignal::sendNotificationToUser($panellab->lab."(Lab) has been requested from ".Auth::user()->getNameOrUsername(), 'a6558cdc-8eae-45f8-8f61-101736d29262', $url = null, $data = null, $buttons = null, $schedule = null);
                                                              // }
                                                              // else
                                                              // {
                                                              //   OneSignal::sendNotificationToUser($panellab->lab."(Radiology) has been requested from ".Auth::user()->getNameOrUsername(), '5697df5c-bdf6-40c0-adf7-df0ddc9f3b39', $url = null, $data = null, $buttons = null, $schedule = null);
                                                              // }
                     ...
                     elseif($getpatientstatus->care_provider == 'Glico Tpa Barclays')
                       ...
                       elseif($getpatientstatus->care_provider == 'Glico Tpa Absa')
                       {
                         $investigation_amount = ServiceCharge::where('type',Input::get("investigation"))->value('glico');
        ...
        case 'Gratis':
          ...
          //  if($investigationtype == 'Laboratory')
          //  {
          //   OneSignal::sendNotificationToUser(Input::get("investigation")."(Lab) has been requested from ".Auth::user()->getNameOrUsername(), 'e21f09f6-e266-435b-8a5b-0be439517385', $url = null, $data = null, $buttons = null, $schedule = null);
          //  }
          //  else
          //  {
 ...
 public function addOcularFinding()
    ...
    {     
           ...
           // if(Input::get("blood_group")){$blood_group =  implode(", ", Input::get("blood_group"));} else {$blood_group = null;}
            ...
            PatientHistory::where('patientid',  Input::get("patient_id"))->delete();
            ROS::where('visit_id',  $deleteid)->delete();
            PatientPE::where('visit_id',  $deleteid)->delete();
            
           $complaints                  = new PatientComplaint;
           $complaints->visitid         = Input::get("opd_number");
           $complaints->complaint       = Input::get("complaint");
           $complaints->period          = Input::get("com_period");
           $complaints->span            = Input::get("com_span");
           $complaints->remark          = Input::get("com_remark");
           $complaints->presenting      = Input::get("presentingcomplaint");
           ...
           $history->created_by                 = Auth::user()->getNameOrUsername();
            ...
            if($complaints->save())
            {
              ...
              $affectedRows = OPD::where('opd_number',Input::get("opd_number"))->update(array('chief_complaint' => Input::get("complaint"),'location'=>'Consulting Room'));
...
public function getInvestigationResults()
{
    ...
    {
            $opd_number = Input::get("opd_number");
            $investigations = LabTestResult::where('labid','=',$opd_number)->where('result','<>',null)->get();
            return  Response::json($investigations);        
    }
...
public function excludeComplain()
   ...
   public function editReview()
    ...
    public function printVisitSummary($id)
    {
        ...
        $mydrugs = Prescription::where('visitid' ,'=', $id)->get();
        $mylabs = PatientInvestigation::where('visitid' ,'=', $id)->get();
        $myoralassessment = PatientAssessment::where('visit_id' ,'=', $id)->get();
        $myplans = PatientPlan::where('visitid' ,'=', $id)->get();
        $mydiagnosis = PatientDiagnosis::where('visitid' ,'=', $id)->get();
        $myvitals = PatientVitals::where('visit_id' ,'=', $id)->orderby('created_on','desc')->get();
        $eyefindings = PatientDiagnosis::where('visitid' ,'=', $id)->get();
    ...
    public function printVisitSummaryDental($id)
    {
         //Timeline
        $admission   = OPD::where('opd_number' , $id)->first();
        $patients    = Customer::where('patient_id' , $admission->patient_id)->first();
        ...
        $myoralassessment = PatientAssessment::where('visit_id' ,'=', $id)->get();
        $myplans = PatientPlan::where('visitid' ,'=', $id)->get();
        $myprocedures = ReferalNote::where('visit_id' ,'=', $id)->get();
        $mydiagnosis = PatientDiagnosis::where('visitid' ,'=', $id)->get();
        $myvitals = PatientVitals::where('visit_id' ,'=', $id)->orderby('created_on','desc')->get();
        $eyefindings = PatientDiagnosis::where('visitid' ,'=', $id)->get();


        return view('dentist.summary', compact('admission','eyefindings','myprocedures','myoralassessment','myplans','patients','mycomplaints','myhistories','myros','mype','mydrugs','mylabs','mydiagnosis','myvitals'));
    }
