          ...
          <section class="vbox bg-white">
             ...
             <div class="page">
              <img src="/images/{{ $mycompany->logo }}" width="15%">
               ...
               <p style="font-size:18px"> Dear {{ $visits[0]->care_provider }}, </p>
                ...
                We submit herewith Medical bill for the period {{ date("jS M Y", strtotime($from))  }} to {{ date("jS M Y", strtotime($to))  }}. We appreciate your clients visit and hope the experience met their expectations. </p>
                <br>
                ...
                <tbody>
                 @foreach($visits as $keys => $visit )
                  ...
                  <tr>
                    ...
                    <td>@foreach($visit->diagonsis as $val) 
                      
                      @if($visits[0]->payercode == 'Corporate') 
                      ##########
                      @else
                        {{ strtoupper($val->diagnosis) }}
                      @endif 
            ...
            <div class="page">
             ...
             <p class="text-dark">Consultation Type :  <label class="badge bg-info"> {{ $visit->consultation_type }} </label> </p>
              <p class="text-dark">Diagnosis or Nature of Illness :  <label class="badge bg-info"> @foreach($visit->diagonsis as $val) 
                 @if($visits[0]->payercode == 'Corporate') 
                      ##########
                      @else
                        {{ strtoupper($val->diagnosis) }}
