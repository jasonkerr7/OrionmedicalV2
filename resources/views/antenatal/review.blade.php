              ...
              {{--     <a href="#new-discharge"  data-toggle="modal" class="btn btn-sm btn-warning bootstrap-modal-form-open pull-right"> <i class="fa fa-plus"></i> Discharge Patient </a> --}}
                    ...
                    <header class="header bg-light bg-gradient">
                     ...
                     <div class="tab-content"> 
                        ...
                        <p class="h4 text-dark"><strong><br> <br> @foreach($myoldlabs as $val)  <label class="badge @if($val->status=="Ready") bg-success @else bg-danger @endif"> {{ $val->created_on->diffForHumans() }} investigations </label> <a href="/laboratory-results-master/{{ $val->visitid }}" target="_blank"><i class="fa fa-folder-o"></i></a>  @endforeach </strong></p> 
                            
                        </div>
                        
                      </section>
                       <img src="/images/212327.svg" width="5%" align="right"> 
                          <section class="panel panel-info">
                                  
                              
                              
                              <header class="panel-heading font-bold">Investigation Requested</header>
                                  <div class="panel-body">
    ...
    function loadInvestigation()
   {
        ...
        $.get('/patient-investigation',
          ...
          { 
            ...
            $.each(data, function (key, value) 
            {           
           $('#investigationsTable tbody').append('<tr><td>'+ value['investigation'] +'</td><td>'+ value['cost'] +'</td><td>'+ value['created_on'] +'</td><td>'+ value['created_by'] +'</td><td><input type="text" style="width:200px; border: 1px solid #ABADB3; text-align: center;" item_code="'+ value['id'] +'" value="'+ value['remark'] +'" onchange="change_count(this);"></td><td>'+ value['status'] +'</td><td>' + ( value['type'] == "Laboratory" ? '<a a href="/test-collection-slip/'+value['visitid']+'">' : '<a a href="/image-request-slip/'+value['id']+'">' ) + '<i onclick="" class="fa fa-print" data-toggle="tooltip" data-placement="top" title="" data-original-title="Print investigation request"></i></a></td><td>' + ( value['type'] == "Laboratory" && value['status'] == "Ready"  ? '<a href="/laboratory-results-master/'+value['result_id']+'" target="_blank">' : '<a a href="#">' ) + '<i onclick="" class="fa fa-eye" data-toggle="tooltip" data-placement="top" title="" data-original-title="View result"></i></a></td><td><a a href="#"><i onclick="removeinvestigation('+value['id']+')" class="fa fa-trash-o" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete Investigation"></i></a></td></tr>');
            });
